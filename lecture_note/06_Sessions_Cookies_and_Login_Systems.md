# Chapter 6. Sessions, Cookies, and Login Systems
...
## 1️⃣ Sessions: Concepts and How They Work (세션의 개념과 작동 원리)

### 1-1 What is a Session? (세션이란?)
...
### 1-2 session_start() Function (session_start() 함수)
...
### 1-3 Storing Data in $_SESSION ($_SESSION에 데이터 저장)
...
## 2️⃣ Cookies: Basics (쿠키의 개념 - 기초)

### 2-1 What is a Cookie? (쿠키란?)
...
### 2-2 setcookie() Function (setcookie() 함수)

**Purpose:** Store a cookie in the browser.
...
### 2-3 When to Use Sessions vs Cookies (세션과 쿠키 사용 시점)
...
## 6️⃣ Key Concepts Summary (주요 개념 정리)

### 6-1 Session Functions Summary (세션 함수 정리)
...
// Initialize session storage on server
// (서버에서 세션 저장소 초기화)

// Enable use of $_SESSION array
// (($_SESSION 배열 사용 가능하게 함)
session_start();

// Store session data (associative array)
// (세션 데이터 저장 - 연관 배열)

// Store data in key-value format
// (Key-Value 방식으로 데이터 저장)

// $_SESSION accessible from other pages
// (다른 페이지에서도 $_SESSION으로 접근 가능)

// Store data independently per user
// (사용자별로 독립적인 데이터 저장)
$_SESSION['key'] = 'value';
$_SESSION['user_id'] = 1;
$_SESSION['username'] = 'John Smith';
...
// isset(): Does variable exist?
// (변수가 존재하는가?)

// Returns true if $_SESSION['key'] exists, false if not
// (존재하면 true, 없으면 false)
if (isset($_SESSION['key'])) {
    echo $_SESSION['key'];
}

// Delete specific session data only
// (특정 세션 데이터만 삭제)

// unset() removes specific element from array
// (배열에서 특정 요소 제거)

// Only $_SESSION['user_id'] is deleted
// (($_SESSION['user_id']만 삭제됨)

// $_SESSION['username'] remains
...
// Remove all $_SESSION information stored on server
// (서버에 저장된 모든 $_SESSION 정보 제거)
...
### 6-2 Password Processing (비밀번호 처리)

```php
<?php

// ===== When registering (Sign up - store password)
// (회원가입할 때 - 비밀번호 저장)

// Plaintext password entered by user
// (사용자가 입력한 평문 비밀번호)
$password = 'password123';

// password_hash(): Safely convert password to hash
// (비밀번호를 안전하게 해시 변환)

// PASSWORD_DEFAULT: Use safest algorithm available
// (현재 가장 안전한 알고리즘 사용)

// Return value: Hashed string (different each time)
// (반환값: 해시된 문자열 - 매번 다른 결과)

// Example: "$2y$10$eImiTXuWVxfaHNYY0iNAeuK2kRabMmwqbF/LewY5D..."
// (예)
$hashed = password_hash($password, PASSWORD_DEFAULT);

// Store in database (hash, not plaintext!)
// (평문이 아닌 해시를 저장!)
// INSERT INTO users (password) VALUES ('$hashed')

// ===== When logging in (verify password)
// (로그인할 때 - 비밀번호 확인)
...
// Hash retrieved from database
// (데이터베이스에서 조회한 해시)
$hashed_from_db = $hashed;  // Hash value from database
                             // (데이터베이스의 해시 값)

// password_verify(): Does entered password match stored hash?
// (입력한 비밀번호가 저장된 해시와 일치하는가?)

// First parameter: User input (plaintext)
// (첫 번째 인자: 사용자 입력 - 평문)

// Second parameter: Hash from database
// (두 번째 인자: 데이터베이스의 해시)
...
### 6-3 Login Verification Pattern (로그인 확인 패턴)
...
// ===== Method 1: Simply check if logged in
// (방법 1: 단순히 로그인 여부 확인)
...
//   (로그인하지 않은 사용자)
if (isset($_SESSION['user_id'])) {
    // This block executes only for logged-in users
    // (이 블록은 로그인한 사용자만 실행됨)
    echo "You are logged in";
    echo $_SESSION['username'];
}

// ===== Method 2: Login required page
// (방법 2: 로그인이 필수인 페이지)

// Block non-logged-in users
// (로그인하지 않은 사용자 차단)

// - !isset(): Session does not exist?
//   (세션이 존재하지 않으면 true)
if (!isset($_SESSION['user_id'])) {
    ...
    // Prevent code below from executing after header()
    // (header()로 페이지 이동 후 이 아래 코드 실행 방지)

    // Important: Always use exit; after header()
    // (중요: header() 후에는 반드시 exit; 사용)
    exit;
}

// Code from here onwards executes only for logged-in users
...
// etc. Security-required features
// (등 보안이 필요한 기능들)
