...
### 1-1 What is an HTML Form? (HTML Form이란?)
...
## 2️⃣ GET vs POST (GET과 POST 비교)

### 2-1 GET Method (GET 방식)
...
### 2-3 GET vs POST Comparison (GET과 POST 비교)

| Item | GET | POST |
| --- | --- | --- |
| **Data Display** | Visible in URL (URL에 표시) | Hidden (숨겨짐) |
| **Security** | Low (낮음) | High (높음) |
| **Purpose** | Query (조회) | Transmission (전송) |
| **Data Size** | Limited (제한있음) | Large (큼) |
| **Caching** | Cached (캐시됨) | Not cached (캐시 안 됨) |
...
### 3-2 isset() and empty() Functions (isset()와 empty() 함수)
...
### 6-1 htmlspecialchars() Function (htmlspecialchars() 함수)

**Purpose:** Convert user input so it is not interpreted as HTML tags.
...
## 8️⃣ Key Concepts Summary (주요 개념 정리)

### 8-1 Basic Form Processing Pattern (폼 처리 기본 패턴)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Step 3: Process if validation passes
    // (단계 3: 검증 통과 시 처리)
    if (count($errors) === 0) {
        $data1 = htmlspecialchars($_POST['field1']);
        $data2 = htmlspecialchars($_POST['field2']);
  
        // Process data (save, modify, delete, etc.)
        // (데이터 처리 - 저장, 수정, 삭제 등)
...
### 8-2 Validation Functions Summary (검증 함수 정리)
...
// (필드 확인)
isset($_POST['field']);      // Does field exist? (필드가 존재하는가?)
empty($_POST['field']);      // Is field empty? (필드가 비어있는가?)

// String validation
// (문자열 검증)
strlen($str) >= 5;           // Check minimum length (최소 길이 확인)
filter_var($email, FILTER_VALIDATE_EMAIL);  // Check email format (이메일 형식)
is_numeric($age);            // Is it a number? (숫자인가?)

// Safe output
// (안전한 출력)
htmlspecialchars($data);     // Convert HTML special characters (HTML 특수문자 변환)

?>
```

### 8-3 Important Security Checklist (보안 체크리스트)

✅ Always validate on the server
(항상 서버에서 검증)

✅ Use htmlspecialchars() for all user input
(모든 사용자 입력에 htmlspecialchars() 사용)
