...
## 1️⃣ File Upload Basics (파일 업로드의 기초)

### 1-1 What is File Upload? (파일 업로드란?)
...
### 3-1 move_uploaded_file() Function (move_uploaded_file() 함수)
...
## 4️⃣ Error Handling (에러 처리)

### 4-1 What is try-catch Block? (try-catch 블록이란?)

**Not stopping program even if error occurs:**

(에러가 발생해도 프로그램을 중단시키지 않기:)

```
try-catch syntax: (try-catch 문법:)
...
### 6-1 File Upload Checklist (파일 업로드 체크리스트)

```
Things to check when uploading files: (파일 업로드 시 확인사항:)

[ ] Set enctype="multipart/form-data" in HTML form
    (HTML 폼에서 enctype="multipart/form-data" 설정)

[ ] Set method="POST" in HTML form
    (HTML 폼에서 method="POST" 설정)

[ ] Add <input type="file"> tag
    (<input type="file"> 태그 추가)

[ ] Verify $_FILES['filename']['error'] === 0
...
[ ] Generate new filename (prevent duplication)
    (새 파일명 생성 - 중복 방지)
...
[ ] Save file information in database (if needed)
    (데이터베이스에 파일 정보 저장 - 필요시)
```

### 6-2 $_FILES Array Structure ($_FILES 배열 구조)

```php
<?php

// $_FILES array structure after file upload
...
$_FILES['upload_file'] = [
    ...
    // Temporary save path (auto-generated)
    // (임시 저장 경로 - 자동 생성)
    'tmp_name' => '/tmp/php123abc',
...
### 6-3 Error Handling Pattern (에러 처리 패턴)

```php
<?php

// Pattern 1: Using try-catch
// (패턴 1: try-catch 사용)
try {
    // Risky operations (file upload, DB access, etc.)
    // (위험한 작업 - 파일 업로드, DB 접근 등)
    if (condition not met) {
        // throw new Exception("message")
        // = Generate error and move to catch block
        // (= 에러를 발생시키고 catch 블록으로 이동)
        // Exception = Object containing error information
    ...
    // Error handling
    // (에러 처리)
    // $e->getMessage() = Return message stored in Exception object
    // ($e->getMessage() = Exception 객체에 저장된 메시지 반환)
    echo "❌ " . $e->getMessage();
}

// Pattern 2: Using if condition
// (패턴 2: if 조건 사용)
...
// Pattern 3: Separate into function
// (패턴 3: 함수로 분리)
function validateFile($file) {
    ...
    return true;  // Validation successful (검증 성공)
}

$result = validateFile($_FILES['upload_file']);
// !== = Are value and type both different?
// (!== = 값이 같지 않고 타입도 다른가?)
// true !== "error message" → true (error occurred)
