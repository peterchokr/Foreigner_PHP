...
## 1️⃣ PHP Basic Syntax (PHP 기본 문법)

### 1-1 PHP Tags and Output (PHP 태그와 출력)
...
#### **var_dump() - Variable Detailed Information (var_dump() - 변수 상세 정보)**
...
$number = 42;
var_dump($number);
// Result: int(42) (결과: int(42))

$text = "Hello";
var_dump($text);
// Result: string(5) "Hello" (결과: string(5) "Hello")

$is_active = true;
var_dump($is_active);
// Result: bool(true) (결과: bool(true))
...
### 1-2 Practice Example: Understanding Basic Syntax (실습 예제: 기본 문법 이해하기)
...
<html>
<head>
    <title>PHP Basic Syntax (PHP 기본 문법)</title>
    <style>
        body {
...
<div class="container">
    <h1>📝 PHP Basic Syntax (PHP 기본 문법)</h1>
    ...
    <!-- Variable output -->
    <div class="section">
        <h2>3️⃣ Variable Output (변수 출력)</h2>
        <?php
    ...
    <!-- var_dump usage -->
    <div class="section">
        <h2>4️⃣ var_dump() - Variable Detailed Information (var_dump() - 변수 상세 정보)</h2>
        <p>You can check the type and value of variables: (변수의 타입과 값을 확인할 수 있습니다:)</p>
...
**When you run this file at `http://localhost/basic_syntax.php`: (이 파일을 `http://localhost/basic_syntax.php`로 실행하면:)**
...
$name = "John Smith";           // String (문자열)
$age = 25;                      // Integer (정수)
$height = 180.5;                // Float (실수)
$is_student = true;             // Boolean (불린)
...
$value = "Hello";      // Changed to string (문자열로 변경)
var_dump($value);      // string(5) "Hello"

$value = 3.14;         // Changed to float (실수로 변경)
var_dump($value);      // float(3.14)

$value = true;         // Changed to boolean (불린으로 변경)
...
if (isset($user_name)) {
    echo "Variable is defined<br>";  // 변수가 정의되었습니다
} else {
    echo "Variable is not defined<br>";  // 변수가 정의되지 않았습니다
}
...
unset($temp);     // Delete variable (변수 삭제)
var_dump($temp);  // ❌ Notice: Undefined variable

?>
```

#### **Defining Constants (상수 (Constant) 정의)**
...
// ============================================
// && vs and, || vs or (우선순위가 다름)
// ============================================

// && and and are almost the same but have different precedence
$x = true && false;     // false
$y = true and false;    // true (and has lower precedence)

// || and or are the same
$a = true || false;     // true
$b = true or false;     // true

// ✅ Recommendation: use && and || (권장: && 와 || 사용)
...
<div class="calculator">
    <h1>🧮 Calculator (계산기)</h1>
  
    <form method="POST">
        <!-- First number -->
        <div class="form-group">
            <label for="num1">First Number: (첫 번째 숫자:)</label>
        ...
        <!-- Operator selection -->
        <div class="form-group">
            <label for="operator">Operator: (연산자:)</label>
        ...
        <!-- Second number -->
        <div class="form-group">
            <label for="num2">Second Number: (두 번째 숫자:)</label>
        ...
        <div class="button-group">
            <button type="submit">Calculate (계산하기)</button>
            <button type="reset">Clear (초기화)</button>
        </div>
    </form>
...
4. **Display calculation history** (계산 이력 표시)
   ...
   - Display previous calculation results in table (세션 또는 배열 사용) / 이전 계산 결과를 테이블로 표시 (세션 또는 배열 사용)
