...
<h2>📝 User Information (사용자 정보)</h2>
<form id="myForm">
    <input type="text" id="name" placeholder="Full Name (이름)" required>
    <input type="email" id="email" placeholder="Email (이메일)" required>
...
<script>
// Handle form submission (폼 제출 처리)
document.getElementById('myForm').addEventListener('submit', function(e) {
    e.preventDefault();  // Prevent default form behavior (기본 폼 동작 방지)
    const name = document.getElementById('name').value;
    const email = document.getElementById('email').value;
    document.getElementById('result').innerHTML = `✅ Name: ${name}, Email: ${email}`;
    document.getElementById('result').style.display = 'block';
    this.reset();  // Clear form (폼 초기화)
});
</script>
...
<h2>🎨 Click the buttons (버튼을 클릭해보세요)</h2>
<button id="btn1">Change Color (색상 변경)</button>
<button id="btn2">Change Size (크기 변경)</button>
<button id="btn3">Change Text (텍스트 변경)</button>
...
// Change dimensions (크기 변경)
document.getElementById('btn2').addEventListener('click', () => {
    document.getElementById('box').style.width = '200px';
    document.getElementById('box').style.height = '200px';
});

// Change text content (텍스트 내용 변경)
...
<h2>📝 Input Verification (입력값 확인)</h2>
<input type="text" id="input1" placeholder="Enter text... (텍스트를 입력하세요...)">

<div id="output">Waiting for input... (입력을 기다리는 중...)</div>

<script>
// Monitor input and display character count (입력 모니터링 및 문자 수 표시)
...
```php
<?php
// Display greeting message (인사말 메시지 표시)
...
<html>
<head>
    <title>Student Grades (학생 성적)</title>
    <style>
...
<h2>📊 Student Grade Management (학생 성적 관리)</h2>

<table>
    <thead>
        <tr><th>Name (이름)</th><th>Score (점수)</th></tr>
    </thead>
    <tbody>
        <!-- Loop through student array and display each row (학생 배열을 반복하며 각 행 표시) -->
...
```sql
-- Create students table (학생 테이블 생성)
...
```sql
-- Insert multiple student records (여러 학생 레코드 삽입)
...
```sql
-- Select all data (모든 데이터 조회)
SELECT * FROM students;

-- Select specific columns with condition (조건을 포함한 특정 열 조회)
...
```sql
-- Update a student's age (학생의 나이 수정)
...
```sql
-- Delete a student record (학생 레코드 삭제)
